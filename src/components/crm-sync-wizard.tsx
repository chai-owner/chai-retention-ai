// CRM sync overlay — pulls live records from a connected CRM (Salesforce,
// HubSpot or Zoho) through a server function, then shows the same editable
// review screen as ChAi Data Drop before anything is saved. Nothing is
// imported until the data is clean and the user confirms.
import { useEffect, useMemo, useState } from "react";
import { toast } from "sonner";
import { useServerFn } from "@tanstack/react-start";
import { AlertTriangle, CheckCircle2, Building2, Loader2, X } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogDescription,
  ...
  function confirmAndSave() {
    ...
    for (const d of datasets) {
      ...
      const findings: QualityFinding[] = [
        { level: "info", text: `Synced from ${providerName} CRM.` },
      ];
      const record: UploadRecord = {
        id: `crm_${provider}_${d.key}_${Date.now().toString(36)}`,
      ...
      <DialogContent className="max-h-[90vh] max-w-4xl overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Building2 className="h-4 w-4 text-primary" /> Sync from {providerName}
          </DialogTitle>
          <DialogDescription>
            Review the accounts and deals ChAi pulled from {providerName}, fix anything that looks
            off, then confirm. Nothing is saved until you import.
          </DialogDescription>
        </DialogHeader>
        ...
        {!loaded && (
          <div className="flex flex-col items-center justify-center gap-3 py-12 text-center">
            ...
            <span className="text-xs text-muted-foreground">
              Fetching accounts and deals through your secure connection.
            </span>
          </div>
        )}
