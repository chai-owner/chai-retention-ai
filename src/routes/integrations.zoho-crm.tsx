...
import { ZohoIcon, ZohoColor } from "@/components/landing/brand-icons";
import { canonicalHead } from "@/lib/site";

const PATH = "/integrations/zoho-crm";
const META_TITLE = "Churn Analytics for Zoho CRM Users | ChAi";
const META_DESCRIPTION =
  "Connect Zoho CRM and see which customers are at risk — and why — in minutes. No spreadsheets, no data team, no setup.";
...
const faqs = [
  ...
  {
    q: "Is my Zoho data secure?",
    a: "Yes — connected through Zoho's official OAuth, encrypted with AES-256, and every sync runs server-side. Your Zoho password never touches ChAi.",
  },
  {
...
export const Route = createFileRoute("/integrations/zoho-crm")({
  head: () => ({
    meta: [
      { title: META_TITLE },
  ...
  component: ZohoCrmIntegrationPage,
});
...
function ZohoCrmIntegrationPage() {
        ...
        <div className="mx-auto mt-10 max-w-[900px] px-6 text-center lg:mt-16 lg:px-8">
          <Reveal>
            <span className="inline-flex items-center gap-2 rounded-[10px] bg-white/10 px-3.5 py-1.5 text-xs font-semibold text-white/85">
              <span className="flex h-5 w-5 items-center justify-center rounded-[6px] bg-white" style={{ color: ZohoColor }}>
                <ZohoIcon className="h-3.5 w-3.5" />
              </span>
      ...
      <section className="mx-auto max-w-[1240px] px-6 py-24 lg:px-8">
        <Reveal className="mx-auto max-w-2xl text-center">
          ...
          <h2 className="mt-4 text-3xl font-extrabold tracking-[-0.02em] sm:text-4xl">
            CRM activity alone doesn't tell the whole story
          </h2>
