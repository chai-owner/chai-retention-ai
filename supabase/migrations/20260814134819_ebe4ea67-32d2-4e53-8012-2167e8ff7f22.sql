-- 1. Pin a fixed search_path on the email queue helpers (previously mutable).
...
-- Qualify pgmq calls now that search_path is empty.
...
-- 2. These SECURITY DEFINER helpers are internal: only the service role / cron
-- should call them. Remove client-facing execute rights.
