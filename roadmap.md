# Roadmap

- [x] Add server-enforced impersonation expiry and audit reasons
- [x] Make impersonation state memory-only and restore admin automatically
- [x] Keep the end control globally visible
- [x] Add tests and verify the app
- [x] Today daily-brief screen (default landing, nav link, summary + top 5 actions)
- [x] Weekly Monday 7am UTC digest email from customer_scores snapshots
- [x] Apply shared UI radius, flat-card, and semantic icon-chip system
- [x] Align Risk Center, Insights, and Settings with shared primitives
- [x] Verify palette discipline, typecheck, build, tests, and responsive pages
- [ ] Add a safe authenticated live AI configuration diagnostic
- [ ] Verify fresh-signup onboarding auth reaches metric generation
- [ ] Run the AI diagnostic, tests, and production build
